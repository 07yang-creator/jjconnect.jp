 ...
 * const supabase = getSupabaseClientWithAuth(env, token)
 * ```
 */
export function getSupabaseClientWithAuth(env: Env, accessToken: string): SupabaseClient {
  if (!env.SUPABASE_URL) {
    throw new Error('SUPABASE_URL is not set in environment variables')
  }
